...
public class SinosoftIa implements SinosoftInterface{
    ...
    public void SituationOne() {
        ...
        //数组开始下标
        int n =ThreadCount;
        for(int i =0;i<=iacMain_ncpbs.size()/10;i++){
             List<IACMain_NCPB> list =null;
            if(n+10>iacMain_ncpbs.size()){
                list = iacMain_ncpbs.subList(n,iacMain_ncpbs.size());
            }else{
                list = iacMain_ncpbs.subList(n,n+10);
            }
            n+=10;
        ...
        System.out.println("统计数量"+tag+"  "+error);
