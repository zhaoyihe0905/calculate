        ...
        for(int i =0;i<=iacMain_ncpbs.size()/n;i++){
            ...
            service.execute(new Runnable() {
                public void run() {
                    ...
                    int err = 0;
                    //获取数据库连接对象
                    Connection conn = JDBCUtil.getConn("ci");
                    try {
                        //遍历数据
                        for (IACMain_NCPB iacMain_ncpb : ThreadList) {
                            ...
                            List<IACMain_NCPX> iacMain_ncpxs = (List<IACMain_NCPX>) CRUDTemplate.newExecuteQuery(conn, selectSql1, new BeanListHandler(IACMain_NCPX.class), iacMain_ncpb.getPolicyConfirmNo());
                            ...
                            if (iacMain_ncpxs == null || iacMain_ncpxs.size() == 0) {
                                try{
                                    ...
                                    String insertSql = "insert into IACMain_NCPPostpone(PolicyConfirmNo,PolicyNo,CompanyCode,StartDate,EndDate,AfterEndDate,NCPStartDate,\n" +
                                            " NCPEndDate,NCPValidDate,PostponeDay,CityCode,LastPoliConfirmNo,LastCityCode,FrameNo,LicenseNo,EngineNo,BusinessType,InputDate,Flag,ValidStatus) \n" +
                                            "values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
                                    int i = CRUDTemplate.newExecuteUpdate(conn, insertSql, iacMain_ncpb.getPolicyConfirmNo(),
                                ...
                                try {
                                    ...
                                    if(iacMain_ncpxs.get(0).getStartDate().getTime()<=(NCPEndDate+86400000)){
                                        Util.ciEndTimeReverse(iacMain_ncpxs);
                                        //若最靠后一张续保保单的止期>=疫情截止日
                                        if(iacMain_ncpxs.get(0).getEndDate().getTime()>=(NCPEndDate+86400000)){
                                            ...
                                            String insertSql = "insert into IACMain_NCPPostpone(PolicyConfirmNo,PolicyNo,CompanyCode,StartDate,EndDate,AfterEndDate,NCPStartDate,\n" +
                                                    " NCPEndDate,NCPValidDate,PostponeDay,CityCode,LastPoliConfirmNo,LastCityCode,FrameNo,LicenseNo,EngineNo,BusinessType,InputDate,Flag,ValidStatus) \n" +
                                                    "values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
                                            int i = CRUDTemplate.newExecuteUpdate(conn, insertSql, iacMain_ncpxs.get(0).getPolicyConfirmNo(),
                                            ...
                                            flag += 1;
                                            //若最靠后一张续保保单的止期<疫情截止日
                                        }else  if(iacMain_ncpxs.get(0).getEndDate().getTime()<(NCPEndDate+86400000)){
                                    ...
                                    }else if(iacMain_ncpxs.get(0).getStartDate().getTime()>(NCPEndDate+86400000)){
                                        //最靠前续保保单起期-疫情截止日天数
                                        long days = (iacMain_ncpxs.get(0).getStartDate().getTime()-(NCPEndDate+86400000))/86400000;
                            ...
                            for (IACMain_NCPB iacMain_ncpb : ThreadList) {
                                ...
                                if (iacMain_ncpxs == null || iacMain_ncpxs.size() == 0) {
                                    try {
                                        ...
                                        if (iacMain_ncpb.getEndDate().getTime() < (NCPEndDate+86400000)) {
                                            //顺延后保单止期:疫情止期+疫情有效期
                                            l = NCPEndDate + 86400000+ (NCPValidDate * 86400000);
                                            AfterEndDate = new Timestamp(l);
                                    ...
                                    if ((iacMain_ncpb.getEndDate().getTime() <= (NCPEndDate+86400000) && iacMain_ncpxs.get(0).getStartDate().getTime() > (NCPEndDate+86400000) && (((iacMain_ncpxs.get(0).getStartDate().getTime() - (NCPEndDate+86400000))/ 86400000) >= NCPValidDate))){
                                        try {
                                            //顺延后保单止期
                                            l = NCPEndDate + 86400000 + (NCPValidDate * 86400000);
                                            AfterEndDate = new Timestamp(l);
                                    ...
                                    } else if((iacMain_ncpb.getEndDate().getTime() > (NCPEndDate+86400000) && iacMain_ncpxs.get(0).getStartDate().getTime() > iacMain_ncpb.getEndDate().getTime() && (((iacMain_ncpxs.get(0).getStartDate().getTime() - iacMain_ncpb.getEndDate().getTime())/ 86400000) >= NCPValidDate))){
                                        try {
                                            //顺延后保单止期
                                            l = iacMain_ncpb.getEndDate().getTime() + (NCPValidDate * 86400000);
                                            AfterEndDate = new Timestamp(l);
                    ...
                    public void run() {
                        ...
                        try {
                            ...
                            for (IACMain_NCPB iacMain_ncpb : ThreadList) {
                                ...
                                List<IACMain_NCPX> iacMain_ncpxs = (List<IACMain_NCPX>)CRUDTemplate.newExecuteQuery(conn,IACMain_NCPXsql, new BeanListHandler(IACMain_NCPX.class), iacMain_ncpb.getPolicyConfirmNo());
                                //判断:一、无续保单
                                if (iacMain_ncpxs == null||iacMain_ncpxs.size()==0) {
                                    try {
                                        //局部变量
